...
contract GovernanceDAO is IGovernanceDAO, Ownable, Pausable {
    ...
    mapping(uint256 => uint256[]) private _campaignProposals; // track proposals per campaign
    mapping(uint256 => mapping(address => uint256))
        private _votingPowerSnapshot; // snapshot: proposalId => voter => voting power at snapshotBlock
    ...
    function resubmitProposal(
        ...
        _proposals[newProposalId] = Proposal({
            ...
            againstVotes: 0,
            abstainVotes: 0,
            totalVotingPower: donationVault.getCampaignEscrowBalance(
                old.campaignId
            ),
            state: ProposalState.Active,
            executeAfter: 0,
