...
interface IImpactNFT {
    ...
    enum DonorTier { Bronze, Silver, Gold }
    ...
    function getDonorNFTs(address donor) external view returns (uint256[] memory tokenIds);
    function getNFTMetadata(uint256 tokenId) external view returns (NFTMetadata memory);
    function getDonorTokenForCampaign(address donor, uint256 campaignId) external view returns (uint256 tokenId);
    function hasMintedForCampaign(address donor, uint256 campaignId) external view returns (bool);
}
